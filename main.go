...
func RotWord(a uint32) uint32 { return a<<8 | a>>24 }
...
func SubWord(a uint32) uint32 {
	return uint32(sbox_en[a>>24])<<24 |
...
func MixColumns(s state) state {
	for i := 0; i < 4; i++ {
		...
		a2 := s[4*i+2]
		a3 := s[4*i+3]
		s[4*i] = multiplication_by_2[a0] ^ multiplication_by_3[a1] ^ a2 ^ a3
		s[4*i+1] = a0 ^ multiplication_by_2[a1] ^ multiplication_by_3[a2] ^ a3
		s[4*i+2] = a0 ^ a1 ^ multiplication_by_2[a2] ^ multiplication_by_3[a3]
		s[4*i+3] = multiplication_by_3[a0] ^ a1 ^ a2 ^ multiplication_by_2[a3]
	}
	return s
}
